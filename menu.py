...
# Configura la API key (siempre debe ir al principio)
# st.secrets["GOOGLE_API_KEY"] debe estar configurada en Streamlit Cloud
genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
model = genai.GenerativeModel('gemini-1.5-flash-latest')

# --- Crea la barra lateral y el menú ---
st.sidebar.title("Menú")
menu_option = st.sidebar.radio(
    "Selecciona una opción:",
    ("Resumidor de Texto", "Bot de Scouts", "Busqueda Manual", "Acerca de")
)

# --- Define el contenido de la página principal según la opción seleccionada ---

# Opción 1: Resumidor de Texto
if menu_option == "Resumidor de Texto":
    st.title("Resumidor de Texto")
    st.write("Pega aquí un texto para obtener un resumen.")

    # Código de tu resumidor (aquí va la lógica que ya tenías)
    # Ejemplo:
    text_to_summarize = st.text_area("Texto a resumir:", height=250)
    if st.button("Resumir texto"):
        if text_to_summarize:
            #model = genai.GenerativeModel('gemini-1.5-flash-latest')
            prompt = "Por favor, resume el siguiente texto en 5 frases clave:"
            response = model.generate_content(prompt + text_to_summarize)
            st.success(response.text)
        else:
            st.error("Por favor, introduce un texto para resumir.")

# Opción 2: Bot de Scouts
elif menu_option == "Bot de Scouts":
    st.title("Bot Experto en Escultismo de Venezuela")
    st.write("Hazme una pregunta sobre la organización scout.")

    # Código de tu bot (aquí va la lógica que ya tenías)
    # Ejemplo:
    scout_question = st.text_input("Haz una pregunta:")
    if st.button("Obtener respuesta"):
        if scout_question:
            model = genai.GenerativeModel('gemini-1.5-flash-latest')
            prompt = """
            Eres un experto en los estatutos de la Asociación de Scouts de Venezuela.
            Responde la siguiente pregunta basándote en la información que tienes.
            Si no tienes la información, di que no puedes responder.
            """
            # Asume que el texto del estatuto está en una variable o en un archivo
            estatuto_text = f"""
...
Grupos Scouts en Carabobo

Carabobo
Grupo Scout Acuarius
gs.acuarius@scouts.org.ve
Carabobo
Grupo Scout Almirante Luis Brión
gs.almiranteluisbrion@scouts.org.ve
Carabobo
Grupo Scout Batalla De Carabobo
gs.batalladecarabobo@scouts.org.ve
Carabobo
Grupo Scout Casupo
gs.casupo@scouts.org.ve
Carabobo
Grupo Scout Don Bosco Valencia
gs.donbosco.valencia@scouts.org.ve
Carabobo
Grupo Scout Exploradores De Brownsea
gs.exploradoresdebrownsea@scouts.org.ve
Carabobo
Grupo Scout Gea
gs.gea@scouts.org.ve
Carabobo
Grupo Scout General Rafael Urdaneta
gs.generalrafaelurdaneta@scouts.org.ve
Carabobo
Grupo Scout Inaguanagoa
gs.inaguanagoa@scouts.org.ve
Carabobo
Grupo Scout Jean Carlos Guevara
gs.jeancarlosguevara@scouts.org.ve
Carabobo
Grupo Scout Júpiter
gs.jupiter@scouts.org.ve
Carabobo
Grupo Scout La Salle Guaparo
gs.lasalleguaparo@scouts.org.ve
Carabobo
Grupo Scout La Salle San José
gs.lasallesanjose@scouts.org.ve
Carabobo
Grupo Scout Libertador Carabobo
gs.libertador.carabobo@scouts.org.ve
Carabobo
Grupo Scout Monseñor Arocha
gs.monsenorarocha@scouts.org.ve
Carabobo
Grupo Scout Nuestra Señora De Begoña
gs.nuestrasenoradebegona@scouts.org.ve
Carabobo
Grupo Scout Padre Jorge Devos
gs.padrejorgedevos@scouts.org.ve
Carabobo
Grupo Scout San Diego De Alcalá
gs.sandiegodealcala@scouts.org.ve
Carabobo
Grupo Scout Valencia Del Rey
gs.valenciadelrey@scouts.org.ve """


# Aquí va el texto completo que usamos
            
            full_prompt = prompt + estatuto_text + "\n\nPregunta: " + scout_question
            response = model.generate_content(full_prompt)
            st.success(response.text)
        else:
            st.error("Por favor, haz una pregunta.")
            

# Opción 3: Página de 'Busqueda'
elif menu_option == "Busqueda Manual":
    st.title("🤖 Asistente de Búsqueda Manual")
    st.write(
    "1. Busca un tema en Internet y copia el texto relevante.\n"
    "2. Pega el texto en el cuadro de abajo.\n"
    "3. Haz una pregunta y el bot responderá usando solo la información que le diste.")

# --- Campos de entrada para el usuario ---
    pasted_text = st.text_area("Pega aquí el texto que encontraste en Internet:", height=300)
    user_question = st.text_input("Haz tu pregunta sobre el texto:")

# --- Botón para obtener la respuesta ---
    if st.button("Obtener Respuesta"):
        if pasted_text and user_question:
        # Construye el prompt completo para el modelo
            full_prompt = (
            f"Eres un asistente experto que responde preguntas. "
            f"Tu tarea es responder la pregunta del usuario basándote **exclusivamente** "
            f"en el texto que se te proporciona a continuación. "
            f"No uses conocimientos externos. Si la respuesta no está en el texto, "
            f"di 'La respuesta no se encuentra en la información proporcionada'."
            f"\n\n--- Texto de referencia ---\n{pasted_text}"
            ...
            st.error("Por favor, pega el texto y haz una pregunta.")

# Opción 4: Página de 'Acerca de'
elif menu_option == "Acerca de":
    st.title("Acerca de")
    st.write("Esta es una aplicación creada con Streamlit y la API de Gemini.")
    st.write("Desarrollada por: PaulaV / JesusP & JoseP.")